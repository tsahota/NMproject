...
It's always best to start with NONMEM template too.  View the code library with:
...
One of the templates is `Models/ADVAN2.mod`.  Preview it with:
...
Use `ls_code_library()` to get the full path of this file and `stage()` to bring it into your project.

```{r eval=FALSE}
#code_library() to view code
ls_code_library("Models/ADVAN2.mod") %>%
  stage()
```

The model will be in the staging area: `staging/Models/ADVAN2.mod`, not `Models/ADVAN2.mod`.  This because we want it to remain an unchanged version of our imported template so that subsequent alteration retains long term reproducibility.

Let's create our first parent NONMEM object with `new_nm()`.  This will require the most set up, subsequent child objects will inherits characterists of the parent.  Three arguments are required to create the parent, a run identifier, `run_id`, a control file it's based on, `based_on`, and a (relative) dataset path `data_path`:
...
A few automatic edits from the staged control file and a compact representation of these changes can be shown by highlighting the above code and selecting the `nm_diff` addin which show what has been changed.  Learning how to read to diffs will be an important skill in NMproject you will pick up over time.  Notice how the `$DATA` has been updated to refer to the new location.
