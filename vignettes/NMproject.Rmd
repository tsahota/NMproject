...
Often there will not be the functions to do the control file manipulation you want.  Although it is preferable to stick to automatic control file manipulation functions, you can do fully tracked `manual edit`s via the addin menu.  Again, just highlight the code, click the addin and follow the instructions:
...
We saw a little at how the functions `init_theta`, `init_omega` and `init_sigma` can be used to set inital estimates earlier.  The functions actually allow manipulation of initial estimates, parameter bounds, names, units, etc.
...
### + That's all well and good that NMproject has it's own boostrap/vpc/... methods, but I just want to run/rerun a bootstrap/vpc/... in PsN.  Can I do this through NMproject?
