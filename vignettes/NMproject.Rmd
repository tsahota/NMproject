...
## Manipulating \$THETA/\$OMEGA/\$SIGMA
...
Note that this is a list of a `data.frame`.  We will now manipulate this with the built in functions like `block` and `unblock` (to create $OMEGA BLOCKS for correlated random effects).

For other manipulations, it is easier in R to work with `data.frame`s.  In that case use, use the function `dplyr::first()` after `init_omega()` will convert it to a `tibble`.  Here it is not needed though.
