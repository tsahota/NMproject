...
### + I have pre-prepared model and dataset. What's the fastest way make an nm object and make use of the objects functionality for post processing etc.
...
### + In my organisation we dont want `execute {ctl_name} -dir={run_dir}` as the default command, how can we change this?

Two options:

1) set it once for your parent object using `cmd` and then make all other objects child objects from the parent.

2) set the `nm.cmd_default` option.  E.g. if you are using docker, you're command is long: `"docker run --rm --user=$(id -u):$(id -g) -v $(pwd):/data -w /data humanpredictions/psn execute {ctl_name} -dir={run_dir}"`.
...
### + How do I set an option so that it's set permanently for all my future R sessions?
