## Description: PK simulation deSolve Depends on: Run interactively: TRUE Keywords:
## deSolve, template, script
...
out <- ldply(1:nrow(params), function(id) {
    ...
    dose.events <- data.frame(var = "Adepot", time = seq(0, end, by = tau), value = dose, 
        method = "add")
    ...
    pkeq <- function(t, y, p) {
        with(as.list(p), {
            ...
            dy1 <- -KA * y[1]
            dy2 <- infusion + KA * y[1] - (CL/V2) * y[2] - (Q/V2) * y[2] + (Q/V3) * 
                y[3]
    ...
    outi <- ode(func = pkeq, times = times, y = yini, parms = params[id, ], events = list(data = dose.events))
...
ds <- ddply(out, "time", summarise, low = quantile(Cblood, probs = 0.025), mid = quantile(Cblood, 
    probs = 0.5), hi = quantile(Cblood, probs = 0.975))

## plot the results
xbreaks <- c(seq(0, 56, by = 7), seq(56, 168, by = 7 * 4))
print(ggplot(ds, aes(x = time, y = mid)) + geom_line() + theme_bw() + geom_ribbon(aes(ymin = low, 
    ymax = hi), fill = "orange", alpha = 0.4) + scale_x_continuous("Time (hrs)") + scale_y_continuous("Plasma concentration (units)"))
