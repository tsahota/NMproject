## Description: compute dollar data statement Depends on: Run interactively: FALSE
## Key words: function
...
dollar_data <- function(d, keep, rename) {
    ...
    if (missing(keep)) {
        ...
        if (length(unmatched) > 0) 
            stop(paste(paste(unmatched, collapse = ","), "not found"))
        keep <- names(d) %in% keep
        keep.nonnumeric <- names(d)[keep & !keep0]
        if (length(keep.nonnumeric) > 0) 
            warning(paste0("Following non-numerics also dropped: \"", paste(keep.nonnumeric, 
                collapse = "\",\""), "\""))
        keep <- keep[!keep %in% keep.nonnumeric]
    }
    n.remaining <- length(which(keep))
    if (n.remaining > 45) 
        warning(paste(n.remaining, "non-dropped variables for nonmem. Consider reducing"))
    # if(length(intersect(names(d)[!keep],names(rename)))>0) stop('can't drop renamed
    # columns')
    ...
    for (i in c("ID", "AMT", "EVID", "CMT", "TIME", "MDV", "RATE", "DATE")) {
        if (i %in% names(d)[!keep]) {
            if (i == "DATE") 
                warning("DATE is dropped, NONMEM will try to use this")
    ...
    for (i in c("EXP", "DEXP", "LOG", "RES", "WRES", "PRED", "IPRED", "PHI")) {
        if (i %in% names(d)) 
            warning(paste(i, "is already defined by NONMEM. Consider renaming"))
    }
    ...
    if (length(cove.vars.missing) > 0) 
        warning(paste0("Core variables not defined: \"", paste(cove.vars.missing, collapse = ","), 
            "\""))
