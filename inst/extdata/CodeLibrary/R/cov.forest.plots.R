## Author: klgk669 First created: 2016-08-27 Description: Forests plots full
## covariate model Depends on: template, function Run interactively: FALSE Keywords:
...
cov.plots <- function(run.no, pars = c("CL", "V", "KA")) {
    ...
    p[[length(p) + 1]] <- ggplot(dc, aes(x = Parameter, y = CHNG)) + theme_bw() + geom_point() + 
        geom_hline(yintercept = 0) + geom_errorbar(aes(ymin = LOW, ymax = UPP)) + scale_y_continuous("% change in parameter (0=no change)", 
        limits = c(-100, 100)) + coord_flip()
