## Author: klgk669 First created: 2016-11-09 Description: Binomial regression
## meta-analysis in R Depends on: Run interactively: TRUE Keywords: binomial,
...
d1 <- data.frame(TRT = 1, nr = c(0.1, 0.3, 0.6), N = c(10, 20, 60))

d2 <- data.frame(TRT = 2, nr = c(0.2, 0.3, 0.8, 0.6, 0.88), N = c(10, 20, 40, 20, 30))
...
p <- ggplot(d, aes(x = STUDY, y = nr, colour = TRT)) + theme_bw() + geom_point() + geom_errorbar(aes(ymin = LOW, 
    ymax = UPP)) + coord_flip()
...
p <- ggplot(d, aes(x = STUDY, y = nr, colour = TRT)) + theme_bw() + geom_point() + geom_errorbar(aes(ymin = LOW, 
    ymax = UPP, linetype = estimated)) + coord_flip()
print(p)
