...
#Created 2017-05-04 at 17:02

rplots.level <- 1
xpose.runno <- '2'
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'execute diagnostic plots run 2'
working.directory<-'/home/klgk669/test2/Models/2/'
raw.results.file <- 'raw_results_run2.csv'
model.directory<-'/home/klgk669/test2/Models/'
model.filename<-'run2.mod'
subset.variable<-NULL
mod.suffix <- '.mod'
mod.prefix <- 'run'
tab.suffix <- ''
theta.labels <- c('KA','K','V','ALAG1','add error','prop error')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)
omega.labels <- c('IIV_KA','IIV_K','IIV_V','IIV_ALAG1')
